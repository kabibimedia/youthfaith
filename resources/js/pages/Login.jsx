...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || err.response?.data?.errors?.login?.[0] || 'Login failed. Please check your credentials.');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="relative bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl p-10 w-full max-w-md mx-4">
                ...
                <div className="flex bg-gray-100 rounded-xl p-1 mb-6">
                    <button onClick={() => setMode('user')}
                        className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${mode === 'user' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                        👤 User Login
                    </button>
                    <button onClick={() => setMode('admin')}
                        className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${mode === 'admin' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                        ⚙️ Admin Login
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <button type="submit" disabled={loading}
                        className={`w-full py-4 text-white font-bold rounded-xl hover:shadow-xl disabled:opacity-50 transition-all text-lg flex items-center justify-center gap-2 ${
                            mode === 'admin'
